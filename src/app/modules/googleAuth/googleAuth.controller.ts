...
export const GoogleAuthController = {
  ...
  googleCallbackRedirect: (req: Request, res: Response) => {
    if (req.session?.user && req.session.token) {
      ...
      res.redirect(
        `http://localhost:3000/google-redirect?token=${req.session.token}`,
      );
    } else {
      console.log(
        'Google login failed after callback. Redirecting to frontend error page: http://localhost:3000/login?error=auth_failed_redirect',
      );
